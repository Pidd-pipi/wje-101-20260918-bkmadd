...
import type { NoteItem, NoteStatus, TastingNote } from '@/constants/note'
...
export function createNote(payload: Partial<TastingNote> & { status?: NoteStatus }) { return request.post<never, TastingNote>('/notes', payload) }
export function updateNote(id: number, payload: Partial<TastingNote>) { return request.put<never, TastingNote>(`/notes/${id}`, payload) }
export function publishNote(id: number) { return request.post<never, TastingNote>(`/notes/${id}/publish`) }
export function deleteNote(id: number) { return request.delete<never, { deleted: boolean }>(`/notes/${id}`) }
export function listMyNotes(status?: NoteStatus) { return request.get<never, TastingNote[]>('/notes/mine', { params: status ? { status } : {} }) }
